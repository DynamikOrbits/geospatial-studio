...
test("blends a vector layer against the map beneath it", async ({ page }) => {
  ...
  // branding-v1 (Geospatial Studio): the default dark basemap can finish its
  // first tile requests after the canvas mounts. Wait for the backdrop itself
  // to stop loading before capturing pixels that must later restore exactly.
...
test("keeps the canvas opaque and the uncovered map intact in every mode", async ({ page }) => {
  await waitForMap(page);
  await page.waitForLoadState("networkidle");
  const backdrop = await settledPixel(page);
  await dropGeoJson(page, "blendtest", POLYGON);
  await expect(layerRow(page, "blendtest")).toBeVisible();
  ...
  for (const lightening of ["screen", "add"]) {
    expect(luminance(rendered.get(lightening)!), `${lightening} did not lighten`).toBeGreaterThan(
      luminance(baseline),
    );
  }
  // `MAX` compares the premultiplied source with the destination. Over the
  // branded dark basemap it lightens the destination as promised, but can be
  // dimmer than ordinary alpha compositing (which adds both contributions).
  expect(
    luminance(rendered.get("lighten")!),
    "lighten did not lighten the backdrop",
  ).toBeGreaterThan(luminance(backdrop));
});
